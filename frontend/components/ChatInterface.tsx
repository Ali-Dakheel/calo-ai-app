...
export function ChatInterface() {
  ...
  const { messages, isLoading, userId, conversationId, addMessage, setLoading, setConversationId } = useChatStore();
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-soft border border-gray-100">
      {/* Header */}
        ...
        {messages.length === 0 && (
          <div className="text-center py-12 space-y-4">
            <div className="w-16 h-16 mx-auto rounded-full bg-calo-light flex items-center justify-center">
              <Bot className="w-8 h-8 text-calo-primary" />
            </div>
            <div>
            ...
            <div className="flex flex-wrap justify-center gap-2 pt-4">
              {[
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm bg-calo-light text-calo-dark rounded-lg hover:bg-gray-100 transition-colors"
                >
                  {suggestion}
                </button>
              ))}
